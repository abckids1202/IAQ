...
export function AuthLogin() {
  ...
  const providerAuth = supabaseConfigured()
  ...
  const sendCode = async () => { if (!email.trim()) { setError('Enter your email first.'); return } setBusy(true); setError(''); try { if (providerAuth) await requestSupabaseOtp(email.trim()); else await requestOtp(email.trim()); setOtpSent(true) } catch (reason) { setError(reason instanceof Error ? reason.message : 'We could not send a code.') } finally { setBusy(false) } }
  const verifyCode = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setError(''); try { if (providerAuth) await verifySupabaseOtp(email.trim(), code.trim()); else await verifyOtp(email.trim(), code.trim()); navigate(safeReturnTo(searchParams.get('returnTo'))) } catch (reason) { setError(reason instanceof Error ? reason.message : 'That code could not be verified.') } finally { setBusy(false) } }
  const googleLogin = () => { void beginSupabaseGoogle().catch((reason) => setError(reason instanceof Error ? reason.message : 'Google sign-in is not configured in this deployment.')) }
  return <div className="access-page"><div className="access-frame"><Link className="access-brand" to="/welcome">IAQ<span>.</span></Link><div className="access-grid"><section className="access-intro"><div className="eyebrow">Sign in / IAQ workspace</div><h1>Keep your profile<br /><em>in your hands.</em></h1><p>Sign in to start a test, read your results, save directions, or continue where you left off.</p><div className="access-note"><strong>{providerAuth ? 'Your sign-in stays with Supabase Auth.' : 'Staff accounts are invitation-only.'}</strong><span>{providerAuth ? 'Email verification and Google sign-in use the configured provider. IAQ receives a verified token, not your password.' : 'Counselor, school, reviewer, and platform access is never created through public signup.'}</span></div></section><section className="access-card"><h2>Continue to IAQ</h2><p className="muted">{providerAuth ? 'Use a verified email code or Google. Your account and role are checked again by the API.' : 'New students can enter an email below to create or reopen a student account. Seeded accounts are for staff workflow previews.'}</p><button className="oauth-button" type="button" onClick={googleLogin} disabled={!providerAuth}><span className="oauth-mark">G</span> Continue with Google {!providerAuth && <small>setup required</small>}</button>{providerAuth ? <><div className="form-divider"><span>or use your email</span></div>{!otpSent ? <div className="otp-form"><label>Email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" autoComplete="email" /></label><button className="button primary full" type="button" onClick={sendCode} disabled={busy}>{busy ? 'Sending…' : 'Send me a code'} <span>→</span></button></div> : <form className="otp-form" onSubmit={verifyCode}><label>Six-digit code<input inputMode="numeric" value={code} onChange={(event) => setCode(event.target.value)} placeholder="123456" maxLength={6} autoComplete="one-time-code" /></label><button className="button secondary full" disabled={busy}>{busy ? 'Checking…' : 'Verify and continue'} <span>→</span></button><small className="muted">Check the inbox for the code from Supabase Auth.</small></form>}</> : <><div className="form-divider"><span>or use a development account</span></div><form onSubmit={login}><label>Account<select value={selected} onChange={(event) => setSelected(event.target.value)}>{demoAccounts.map((account) => <option key={account.id} value={account.id}>{account.label} — {account.detail}</option>)}</select></label><button className="button primary full" disabled={busy}>{busy ? 'Signing in…' : 'Sign in'} <span>→</span></button></form><div className="form-divider"><span>New or returning student · email sign-in</span></div>{!otpSent ? <div className="otp-form"><label>Email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" /></label><button className="button ghost" type="button" onClick={sendCode} disabled={busy}>Send me a code <span>→</span></button><small className="muted">A new email creates a student account automatically.</small></div> : <form className="otp-form" onSubmit={verifyCode}><label>Six-digit code<input inputMode="numeric" value={code} onChange={(event) => setCode(event.target.value)} placeholder="123456" maxLength={6} /></label><button className="button secondary" disabled={busy}>Verify and continue <span>→</span></button><small className="muted">Development code: 123456</small></form>}</>}{error && <p className="form-error" role="alert">{error}</p>}<small className="legal-copy">By continuing, you agree to the draft <Link to="/privacy">privacy model</Link>. Production role checks remain server-side.</small></section></div></div></div>
}

export function AuthCallback() {
  const navigate = useNavigate()
  const [error, setError] = useState('')
  useEffect(() => { void finishSupabaseCallback().then((complete) => { if (complete) navigate('/') }).catch((reason) => setError(reason instanceof Error ? reason.message : 'The sign-in callback could not be completed.')) }, [navigate])
  return <div className="access-page"><div className="access-card standalone"><div className="eyebrow">Authentication callback</div><h1>{error ? 'Sign-in needs another try.' : 'Finishing sign-in…'}</h1><p>{error || 'We are securely returning you to your IAQ workspace.'}</p>{error && <Link className="button primary" to="/auth/login">Back to sign in <span>→</span></Link>}</div></div>
}
