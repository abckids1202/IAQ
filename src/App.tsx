...
function Results({ profile }: { profile: Profile; savedMajors: string[]; toggleMajor: (name: string) => void }) {
  ...
  const [loading, setLoading] = useState(!profile.lastResult && Boolean(resultId))
  useEffect(() => {
    if (!resultId) return
    let active = true
    setLoading(true)
  ...
  if (!result) return <div className="page results-empty"><PageIntro eyebrow="Your results / Private report" title={<>Your report starts<br /><em>after your test.</em></>} body="Complete the 35-minute assessment to see your actual answers, timing, and six-domain profile." action={<Link className="button primary" to="/assess">Start test <span>→</span></Link>} /><section className="panel empty-report-panel"><div className="eyebrow">No scored result yet</div><h2>Nothing to compare yet.</h2><p>IAQ will never fill this report with a demo score. Your bars appear after a real session is submitted.</p><Link className="text-button" to="/methodology">Read how scoring works ↗</Link></section></div>
  if (identityRequired) return <div className="page results-page"><PageIntro eyebrow="Your results / Private report" title={<>Your test is done.<br /><em>Let’s keep it private.</em></>} body="Add the minimum details needed to attach this result to you. You can read the score immediately after this step." /><IdentityCaptureCard resultId={result.id} onComplete={async () => { const refreshed = await getAssessmentResult(result.id); setLiveResult(refreshed); setIdentityRequired(Boolean(refreshed.identityRequired)) }} /></div>
