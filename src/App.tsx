...
import { finishAssessment, getAIStatus, getInterestResult, requestAIDirections, requestAIInterpretation, requestReportDelivery, resumeRandomizedAssessment, saveAndGetNext, startRandomizedAssessment, submitFeedback, type AIDirectionContext, type AIInterpretation, type InterestResult } from './api'
...
function AssessLanding() {
  return <div className="page assess-page"><section className="compact-hero"><div><div className="eyebrow">IAQ / timed assessment</div><h1>Take a test.<br /><em>See your thinking profile.</em></h1><p>56 medium-to-hard questions across seven thinking areas. You have 35 minutes and your report appears as soon as you finish.</p></div><Link to="/assess/session" className="button primary">Start test <span>→</span></Link></section><div className="notice-bar subtle compact-notice"><span className="notice-icon">!</span><span><strong>Before you begin.</strong> Find a quiet place. The clock keeps running if you leave or refresh.</span></div><section className="assessment-overview compact-overview"><div className="assessment-hero"><div className="assessment-index">56 <span>questions</span></div><h2>Seven kinds of thinking.<br /><em>One useful snapshot.</em></h2><p>There is no single “smart” way to think. The report shows how your answers compare across domains.</p><Link to="/methodology" className="text-button light">Read the method ↗</Link></div><div className="domain-list">{domains.map((domain, index) => <div className="domain-row" key={domain}><span className="domain-no">0{index + 1}</span><div><strong>{domain}</strong><span>{domainMeta[domain].description}</span></div><span className={`domain-tag ${domainMeta[domain].tone}`}>8 questions</span></div>)}</div></section></div>
}
...
function ReportDeliveryCard({ resultId }: { resultId?: string }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setStatus('')
    try {
      await requestReportDelivery(resultId, { name, email, age: age ? Number(age) : undefined, granted })
