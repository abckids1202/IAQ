...
function MemoryTrial({ question, selected, onChoose, onBeginRecall, sessionId, disabled }: { question: Question; selected?: string; onChoose: (option: string) => void; onBeginRecall?: () => Promise<void>; sessionId?: string | null; disabled: boolean }) {
  ...
  if (phase === 'study') return <div className="memory-trial memory-study" aria-live="polite"><div className="memory-study-head"><span className="eyebrow">Study stimulus</span><span className="memory-reference">Timing protocol reference only</span></div>{question.imageUrl ? <img className="memory-image" src={question.imageUrl} alt="Working-memory stimulus" /> : <div className="memory-stimulus" aria-label="Working-memory stimulus">{visual.length > 1 ? visual.map((part, index) => <span key={`${part}-${index}`}>{part}</span>) : <strong>{visual[0] || 'Stimulus unavailable.'}</strong>}</div>}<button className="button primary" onClick={async () => { try { await onBeginRecall?.(); if (recallKey) sessionStorage.setItem(recallKey, 'started'); setPhase('recall') } catch { /* parent displays the request error */ } }} disabled={disabled}>Begin recall <span>→</span></button><small>Manual study mode is for review only and is not standardized.</small></div>
  if (stagedProtocol === 'ordered_sequence') {
    const digits = parseResponse().join('').slice(0, question.memoryInputLength || 0)
    return <div className="memory-trial memory-recall"><div className="memory-step-index">02</div><div className="memory-recall-copy"><span className="eyebrow">Recall</span><h2>Enter the sequence you remember.</h2><p>The study card is hidden. Type all {question.memoryInputLength} digits in the original order.</p></div><input className="memory-sequence-input" value={digits} onChange={(event) => { const next = event.target.value.replace(/\D/g, '').slice(0, question.memoryInputLength || 0); onChoose(next ? JSON.stringify(next.split('').map(Number)) : '') }} inputMode="numeric" autoComplete="off" aria-label="Recalled digit sequence" disabled={disabled} /><small className="memory-input-hint">{digits.length} / {question.memoryInputLength} digits entered</small></div>
  }
...
function Results({ profile }: { profile: Profile; savedMajors: string[]; toggleMajor: (name: string) => void }) {
  ...
  useEffect(() => {
    if (!resultId) return
    setLoading(true)
    void getAssessmentResult(resultId).then((next) => { setLiveResult(next); setIdentityRequired(Boolean(next.identityRequired)) }).catch(() => setLiveResult(undefined)).finally(() => setLoading(false))
  }, [resultId])
