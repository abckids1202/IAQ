...
const searchPages = [
  ...
  { to: '/results', label: 'Your results', description: 'Read your six-domain report.' },
...
function App() {
  ...
  useEffect(() => {
    ...
    if (resultId) void getAssessmentResult(resultId).then(applyResult).catch(() => setProfile(initialProfile))
    else setProfile(initialProfile)
  ...
  return <><AppBootLoader active={booting} /><ScrollProgress /><CursorFollower /><Routes>
    ...
    <Route path="/compass" element={<StudentAppShell profile={profile}><PaidDirections resultId={profile.lastResult?.id} /></StudentAppShell>} />
...
function PublicSite() {
  const { t } = useI18n()
  return <div className="public-site"><PublicNavbar /><main className="public-main"><section className="public-hero-grid"><div className="public-hero-copy"><div className="public-kicker">Cara yang lebih jelas untuk mulai memikirkan langkah berikutnya</div><h1>{t('seeHowYouThink')}<br /><em>Then choose what to try.</em></h1><p>IAQ adalah asesmen kognitif berwaktu untuk pelajar yang ingin memiliki bukti sebelum menjelajahi pelajaran, jurusan, dan arah masa depan.</p><div className="public-actions-large"><Link to="/assess" className="button primary">{t('startTest')} <span>→</span></Link><Link to="/methodology" className="text-button">{t('publicHow')} ↗</Link></div></div><aside className="public-hero-proof"><span className="eyebrow">Rilis pertama</span><strong>48</strong><span>soal dalam enam area berpikir</span><div className="public-proof-rule" /><div className="public-proof-meta"><span>35 menit</span><span>laporan pribadi</span><span>hanya dalam profil</span></div></aside></section><section className="public-flow" id="how"><div className="public-flow-intro"><span className="eyebrow">Urutan sederhana</span><h2>Test first. Results next.</h2><p>Arah datang setelah bukti, sehingga langkah berikutnya punya dasar yang nyata.</p></div><div className="public-flow-steps"><article><b>01</b><h3>{t('navTest')}</h3><p>Jawab 48 soal tingkat menengah hingga sulit dalam satu sesi terarah.</p></article><article><b>02</b><h3>{t('navResults')}</h3><p>Lihat enam skor area, garis rata-rata, akurasi, dan konteks waktu.</p></article><article id="schools"><b>03</b><h3>{t('navDirections')}</h3><p>Gunakan profil dan minat Anda untuk memilih langkah kecil yang bisa dicoba.</p></article></div></section><section className="public-evidence"><div><span className="eyebrow">Dibuat untuk kemajuan yang jujur</span><h2>Satu skor bukan seluruh cerita.</h2></div><p>IAQ menjaga hasil tetap pribadi dan sementara. Gunakan untuk refleksi dan percakapan, bukan diagnosis atau peringkat.</p><Link to="/methodology" className="text-button">{t('publicMethodology')} ↗</Link></section></main></div>
}
...
function LegacyHome({ profile, savedMajors }: { profile: Profile; savedMajors: string[] }) {
  return <div className="page">
    ...
    <div className="dashboard-grid">
      <section className="panel profile-panel span-7"><div className="panel-top"><div><div className="eyebrow">Your thinking profile</div><h2>How your mind works today</h2></div><span className="mono-label">IAQ-COG-0.4</span></div><div className="profile-summary"><div className="composite"><span>IAQ IQ score · experimental</span><strong>{profile.iqScore ?? '—'}<span>{profile.iqScore === null || profile.iqScore === undefined ? 'not ready' : 'reference'}</span></strong><small>{profile.confidence || 'moderate'} confidence</small></div><div className="profile-chart"><ResponsiveContainer width="100%" height={190}><AreaChart data={domains.map((domain) => ({ name: domainMeta[domain].short, score: profile.scores[domain] }))} margin={{ left: 0, right: 0, top: 10, bottom: 0 }}><CartesianGrid vertical={false} stroke="#e1ded5" strokeDasharray="2 4" /><XAxis dataKey="name" tickLine={false} axisLine={false} tick={{ fill: '#777973', fontSize: 10 }} /><YAxis domain={[0, 100]} hide /><Tooltip contentStyle={{ border: '1px solid #d7d2c7', borderRadius: 0, boxShadow: 'none', fontSize: 12 }} /><Area type="monotone" dataKey="score" stroke="#315CFF" fill="#e5ebff" strokeWidth={2} /></AreaChart></ResponsiveContainer></div></div><div className="panel-footer"><span>What stands out: <b>{profile.strengths.join(' + ')}</b></span><Link to="/results">See the full picture ↗</Link></div></section>
...
function Home({ profile }: { profile: Profile; savedMajors: string[] }) {
  ...
  const iqScore = profile.iqScore ?? null
  ...
  return <div className="page home-page">
    ...
    <div className="dashboard-grid home-grid"><section className="panel home-result-preview span-8"><div className="panel-top"><div><div className="eyebrow">{hasResult ? `${t('navResults')} / six areas` : 'Your result / six areas'}</div><h2>{hasResult ? t('clearPicture') : t('yourResultAppears')}</h2></div><span className="mono-label">{hasResult ? profile.lastAssessment : 'NOT STARTED'}</span></div><div className="home-result-body"><div className="home-score"><span>{hasResult ? 'IAQ IQ score · experimental' : t('completeTest')}</span><strong>{hasResult && iqScore !== null ? <AnimatedNumber value={iqScore} /> : '—'}{hasResult && iqScore !== null && <small>reference</small>}</strong><p>{hasResult ? `${profile.confidence || 'moderate'} confidence · non-normed experimental result` : t('sevenScores')}</p></div><div className="home-bars" aria-label="Thinking profile preview">{ranked.map((domain) => <div className="home-bar-row" key={domain}><span>{domainMeta[domain].short}</span><i><b className={domainMeta[domain].tone} style={{ width: `${hasResult ? (profile.scores[domain] ?? 0) : 0}%` }} /></i><strong>{hasResult ? (profile.scores[domain] ?? '—') : '—'}</strong></div>)}</div></div><div className="panel-footer"><span>{hasResult ? `Strongest today: ${profile.strengths.join(' + ')}` : t('testFacts')}</span><Link to={hasResult ? '/results' : '/assess'}>{hasResult ? 'Read the full report ↗' : 'See the test details ↗'}</Link></div></section><aside className="panel home-start-panel span-4"><div className="eyebrow">{t('firstStep')}</div><div className="home-step-number">01</div><h2>{hasResult ? 'Now add context.' : t('readyToStart')}</h2><p>{hasResult ? 'Interests and real experiences are more useful after your result gives you a starting point.' : 'Answer carefully. The test chooses a balanced set from the question bank and submits when time runs out.'}</p><Link className="button secondary full" to={hasResult ? '/compass' : '/assess'}>{hasResult ? t('exploreNext') : t('startTest')} <span>→</span></Link></aside><section className="home-sequence span-12"><div><span className="eyebrow">A simple order</span><h2>{t('testFirst')}</h2></div><div className="sequence-steps"><Link to="/assess"><b>01</b><span>{t('navTest')}</span><small>35 minutes / 48 questions</small></Link><Link to="/results"><b>02</b><span>{t('resultsNext')}</span><small>Six visual domain scores</small></Link><Link to="/compass"><b>03</b><span>{t('exploreNext')}</span><small>Use evidence, interests, and curiosity</small></Link></div></section></div>
  </div>
}
...
function LegacyAssessment({ onComplete }: { onComplete: (scores: Record<Domain, number>) => void }) {
  ...
  const totalQuestions = usingApi ? 48 : questions.length
...
function MemoryTrial({ question, selected, onChoose, onBeginRecall, sessionId, disabled }: { question: Question; selected?: string; onChoose: (option: string) => void; onBeginRecall?: () => Promise<void>; sessionId?: string | null; disabled: boolean }) {
  const recallKey = sessionId ? `iaq-memory-recall:${sessionId}:${question.id}` : ''
  const [phase, setPhase] = useState<'intro' | 'study' | 'recall'>(() => question.memoryRecallStarted || (recallKey && sessionStorage.getItem(recallKey) === 'started') ? 'recall' : 'intro')
  ...
  useEffect(() => {
    setPhase(question.memoryRecallStarted || (recallKey && sessionStorage.getItem(recallKey) === 'started') ? 'recall' : 'intro')
  }, [question.id, recallKey])
  if (phase === 'intro') return <div className="memory-trial memory-intro"><div className="memory-step-index">01</div><div><span className="eyebrow">Working memory / study first</span><h2>Study the material carefully.</h2><p>When you are ready, select “Begin recall.” The stimulus will disappear and cannot be replayed.</p></div><button className="button secondary" onClick={() => setPhase('study')} disabled={disabled}>Show stimulus <span>→</span></button></div>
if (phase === 'study') return <div className="memory-trial memory-study" aria-live="polite"><div className="memory-study-head"><span className="eyebrow">Study stimulus</span><p>{question.prompt}</p><span className="memory-reference">Timing protocol reference only</span></div><MemoryStudyStimulus visual={question.visual} /><button className="button primary" onClick={async () => { try { await onBeginRecall?.(); if (recallKey) sessionStorage.setItem(recallKey, 'started'); setPhase('recall') } catch { /* parent displays the request error */ } }} disabled={disabled}>Begin recall <span>→</span></button><small>Manual study mode is for review only and is not standardized.</small></div>
  if (stagedProtocol === 'ordered_sequence') {
    return <div className="memory-trial memory-recall"><div className="memory-step-index">02</div><div className="memory-recall-copy"><span className="eyebrow">Recall</span><h2>{question.prompt}</h2><p>The study card is hidden. Enter {question.memoryInputLength} numbers in the requested order.</p></div><MemorySequenceInput itemId={question.id} length={question.memoryInputLength || 0} onChoose={onChoose} disabled={disabled} /></div>
  }
  if (stagedProtocol === 'cell_set') {
    const cells = parseResponse()
    const size = question.memoryGridSize || 5
    return <div className="memory-trial memory-recall"><div className="memory-step-index">02</div><div className="memory-recall-copy"><span className="eyebrow">Recall</span><h2>Select the cells you remember.</h2><p>The study grid is hidden. Select all {question.memoryInputLength} filled cells.</p></div><div className="memory-recall-grid" style={{ gridTemplateColumns: `repeat(${size}, 42px)` }} role="group" aria-label={`Empty ${size} by ${size} recall grid`}>{Array.from({ length: size * size }, (_, index) => <button type="button" key={index} className={`memory-cell ${cells.includes(index) ? 'selected' : ''}`} onClick={() => { const next = cells.includes(index) ? cells.filter((value) => value !== index) : [...cells, index].sort((a, b) => a - b); onChoose(next.length ? JSON.stringify(next) : '') }} aria-pressed={cells.includes(index)} disabled={disabled} aria-label={`Cell ${index + 1}`} />)}</div><small className="memory-input-hint">{cells.length} / {question.memoryInputLength} cells selected</small></div>
  }
...
function Assessment({ onComplete }: { onComplete: (result: AssessmentResult) => void }) {
  ...
  const begin = async () => {
    ...
    try {
      ...
      const ageBand = (params.get('age_band') || localStorage.getItem('iaq-age-band') || '18-22') as '15-17' | '18-22' | 'adult' | 'unknown'
      const language = (params.get('language') === 'id' ? 'id' : 'en') as 'en' | 'id'
      if (savedSessionId) {
        try {
          started = await resumeRandomizedAssessment(savedSessionId)
        } catch (error) {
          if (error instanceof CompletedAssessment) {
            window.sessionStorage.removeItem(ASSESSMENT_SESSION_KEY)
            if ('practice' in error.result) setPracticeComplete(error.result)
            else { onComplete(error.result); navigate(`/results?result=${encodeURIComponent(error.result.id)}`) }
            return
          }
          if (!(error instanceof ApiError) && !(error instanceof InvalidAssessmentSession)) throw error
          // A browser can retain a session from the old 12-question prototype
          // after an API restart or deployment. Discard that stale pointer and
          // create the enforced 48-question form instead of trapping the user.
          window.sessionStorage.removeItem(ASSESSMENT_SESSION_KEY)
  ...
  const finalize = async () => {
    ...
    try {
      ...
      onComplete(result)
      navigate(`/results?result=${encodeURIComponent(result.id)}`)
    } catch (error) {
  ...
  const beginRecall = async () => {
    if (!apiSession || !question || question.type !== 'memory') return
    setApiError('')
    try {
      await beginMemoryRecall(apiSession, question.id)
    } catch (error) {
      const message = error instanceof Error ? error.message : 'We could not begin the recall step.'
      setApiError(message)
      throw error
    }
  }
...
function IdentityCaptureCard({ resultId, onComplete }: { resultId: string; onComplete: () => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setStatus('')
    try {
      await captureIdentity({ result_id: resultId, email, display_name: displayName, age_band: ageBand, granted })
      onComplete()
    } catch (error) {
...
function Results({ profile }: { profile: Profile; savedMajors: string[]; toggleMajor: (name: string) => void }) {
  ...
  const resultId = searchParams.get('result') || (typeof window !== 'undefined' ? localStorage.getItem('iaq-last-result-id') : null)
  const [liveResult, setLiveResult] = useState<AssessmentResult | undefined>()
  const [history, setHistory] = useState<AssessmentResult[]>([])
  const [historyLoading, setHistoryLoading] = useState(!resultId)
  const [historyError, setHistoryError] = useState('')
  const [loadError, setLoadError] = useState('')
  const [identityRequired, setIdentityRequired] = useState(Boolean(profile.lastResult?.identityRequired))
  const [loading, setLoading] = useState(Boolean(resultId))
  useEffect(() => {
    if (!resultId) {
      let active = true
      setHistoryLoading(true)
      void listAssessmentResults().then((items) => { if (active) setHistory(items) }).catch((error) => { if (active) setHistoryError(error instanceof Error ? error.message : 'Your saved results could not load.') }).finally(() => { if (active) setHistoryLoading(false) })
      return () => { active = false }
    }
    let active = true
    setLoading(true)
    setLiveResult(undefined); setLoadError('')
    void getAssessmentResult(resultId).then((next) => { if (active) { setLiveResult(next); setIdentityRequired(Boolean(next.identityRequired)) } }).catch((error) => { if (active) setLoadError(error instanceof Error ? error.message : 'Your report could not load.') }).finally(() => { if (active) setLoading(false) })
    return () => { active = false }
  }, [resultId])
  const result = liveResult
  if (loadError) return <div className="page results-empty"><section className="panel"><h1>Your result could not load.</h1><p role="alert">{loadError}</p><button className="button secondary" onClick={() => window.location.reload()}>Try again</button><p><Link to="/auth/login">Sign in to the account that owns this result →</Link></p></section></div>
  if (loading) return <div className="page results-empty"><section className="panel loading-card"><div className="eyebrow">Your results / Loading</div><h1>Preparing your report…</h1><p>We are retrieving the saved assessment result.</p></section></div>
  if (!result && !resultId) return <div className="page results-page"><PageIntro eyebrow="Your results / Private report" title={<>Your saved results.<br /><em>One clear place.</em></>} body="Completed assessments stay attached to your account. Open one to see its score, access, and report status." action={<Link className="button primary" to="/assess">Start test <span>→</span></Link>} />{historyLoading ? <section className="panel loading-card"><h2>Loading your saved results…</h2></section> : historyError ? <section className="panel"><p role="alert">{historyError}</p></section> : history.length ? <section className="panel result-history-list" aria-label="Saved assessment results">{history.map((item) => <Link className="result-history-row" to={`/results?result=${encodeURIComponent(item.id)}`} key={item.id}><span><strong>IAQ IQ score — experimental</strong><small>{new Date(item.completedAt).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })} · {item.assessmentVersion}</small></span><b>{item.iqScore ?? '—'}</b><span aria-hidden="true">→</span></Link>)}</section> : <section className="panel empty-report-panel"><div className="eyebrow">No saved assessment</div><h2>Nothing to compare yet.</h2><p>Complete the 48-question assessment to create your first result.</p><Link className="button primary" to="/assess">Start test <span>→</span></Link></section>}</div>
  if (!result) return <div className="page results-empty"><section className="panel"><h1>Your result could not load.</h1><p>Open the result link again or return to your saved results.</p><Link className="button secondary" to="/results">Open saved results <span>→</span></Link></section></div>
  if (identityRequired) return <div className="page results-page"><PageIntro eyebrow="Your results / Private report" title={<>Your test is done.<br /><em>Here is your score.</em></>} body="Your free score is ready now. Add your details below if you want to save this result, pay for the selected report, and return to it later." /><section className="panel cognitive-panel" aria-labelledby="guest-score-title"><div className="eyebrow">IAQ IQ score · experimental</div><h1 id="guest-score-title" className="result-score">{result.iqScore ?? '—'}</h1><p>{result.disclaimer}</p><Link className="text-button" to="/methodology">How this score is calculated ↗</Link></section><IdentityCaptureCard resultId={result.id} onComplete={async () => { const refreshed = await getAssessmentResult(result.id); setLiveResult(refreshed); setIdentityRequired(Boolean(refreshed.identityRequired)) }} /></div>
  if (!result.fullAccess) return <div className="page results-page"><section className="panel cognitive-panel" aria-labelledby="free-score-title"><div className="eyebrow">Your completed assessment</div><h1 id="free-score-title">IAQ IQ score — experimental</h1><div className="result-score">{result.iqScore ?? '—'}</div><p>{result.iqScore == null ? 'Not enough answers to calculate this result. No payment is needed; try a new assessment when you are ready.' : 'This is an uncalibrated test score, not a validated IQ measurement.'}</p><p>{result.disclaimer}</p><div className="secondary-actions"><Link className="text-button" to="/methodology">How this score is calculated ↗</Link><Link className="text-button" to={`/auth/login?returnTo=${encodeURIComponent(`/results?result=${result.id}`)}`}>Save my results ↗</Link></div></section>{result.iqScore != null && <section className="panel paywall-card"><div><div className="eyebrow">This report only · one-time purchase</div><h2>Understand your strongest areas.</h2><p>Unlock your six-domain bars, evaluation, interest check-in, possible directions and private report. Payment does not change your score.</p></div><Link className="button primary" to={`/checkout/iaq-complete?result=${encodeURIComponent(result.id)}`}>Unlock report · Rp50,000 <span>→</span></Link></section>}</div>
  ...
  return <div className="page results-page">
    <PageIntro eyebrow="Your results / Private report" title={<>See your score.<br /><em>Know your strongest areas.</em></>} body="Your experimental IAQ IQ score comes first. The profile below shows which thinking areas stood out in this attempt." action={<button className="button primary" onClick={() => window.print()}>Print my report <span>↗</span></button>} />
    <div className="result-disclaimer"><div className="disclaimer-mark">!</div><div><strong>IAQ IQ score · experimental / {result.iqScore ?? 'insufficient evidence'}</strong><p>{result.disclaimer}</p></div><Link to="/methodology">Read the method ↗</Link></div>
    <div className="result-grid results-top-grid"><section className="panel cognitive-panel span-8"><div className="result-score-head"><div><div className="eyebrow">IAQ IQ score · experimental</div><div className="result-score">{result.iqScore ?? '—'}<span>{result.iqScore === null || result.iqScore === undefined ? 'not enough evidence' : 'reference score'}</span></div><small className="score-method-note">Uncalibrated 70–130 transform · not norm-derived</small></div><div className="result-meta"><span className="confidence-badge">{confidence} confidence</span><span>{new Date(result.completedAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span><span>{result.assessmentVersion}</span></div></div><div className="profile-distribution" aria-label="Strength signals across six thinking areas"><div className="distribution-scale"><span>0</span><span>50</span><span>100</span></div>{ranked.map((domain) => { const value = scores[domain]; return <div className="distribution-row" key={domain}><div className="distribution-label"><span className={`domain-swatch ${domainMeta[domain].tone}`} /><strong>{domain}</strong><b>{value ?? 'Not assessed'}</b></div><div className="distribution-track"><i className={domainMeta[domain].tone} style={{ width: `${value ?? 0}%` }} /><span className="distribution-average" style={{ left: `${composite ?? 0}%` }} aria-hidden="true" /></div><div className="distribution-detail"><span>{value === null || value === undefined ? 'More evidence needed' : (value >= (composite ?? 0) ? 'Above your profile average' : 'Below your profile average')}</span></div></div> })}</div><div className="result-caption"><span><i className="legend-dot cobalt" /> strength signals / 100</span><span><i className="legend-line" /> your profile average / 100</span></div></section><aside className="panel result-context"><div className="eyebrow">Your strongest areas</div><h2>What stands out</h2><p className="result-lede"><strong>{strongest}</strong> {scored.length ? 'came through as the strongest signals in this attempt.' : 'need more evidence before comparison is useful.'}</p><div className="result-strength-list" aria-label="Strongest thinking areas">{scored.slice(0, 3).map((domain, index) => <div className="result-strength-item" key={domain}><span>0{index + 1}</span><strong>{domain}</strong><b>{scores[domain] ?? '—'}</b></div>)}</div><div className="result-observation"><span className="observation-mark coral">→</span><div><strong>Keep exploring</strong><p>{lower ? `${lower} is the lowest relative signal today. Treat it as a question to explore, not a verdict.` : 'Every area needs more evidence in this result.'}</p></div></div><div className="result-context-foot"><span>Profile average: {composite ?? '—'} / 100</span><strong>No population percentile shown.</strong></div></aside></div>
    <section className="result-evidence-strip"><div><span className="eyebrow">Profile shape</span><strong>Six areas</strong><span>shown side by side</span></div><div><span className="eyebrow">Time limit</span><strong>{result ? `${Math.ceil(result.durationSeconds / 60)} minutes` : '35 minutes'}</strong><span>timing is context only</span></div><div><span className="eyebrow">How to read this</span><strong>Notice your pattern</strong><span>not a comparison with others</span></div></section>
    {!result.paidFeaturesUnlocked ? <section className="panel paywall-card"><div><div className="eyebrow">Optional full report</div><h2>Unlock directions and a private copy.</h2><p>Your free result is complete. A one-time unlock adds the interest check-in, explainable direction ideas, completion certificate, and private report delivery.</p></div><Link className="button secondary" to={`/checkout/iaq-complete?result=${encodeURIComponent(result.id)}`}>See unlock options <span>→</span></Link></section> : <AIInterpretationCard resultId={result.id} />}<div className="result-grid result-secondary-grid"><section className="panel secondary-result-panel span-7"><div className="eyebrow">Next, when you are ready</div><h2>Explore what fits you.</h2><p>After your thinking profile, interests and real experiences can add useful context to possible directions. They do not change this result.</p><div className="secondary-actions">{result.paidFeaturesUnlocked ? <Link className="button secondary" to={`/interests?result=${encodeURIComponent(result.id)}`}>Tell us what you enjoy <span>→</span></Link> : <Link className="button secondary" to={`/checkout/iaq-complete?result=${encodeURIComponent(result.id)}`}>Unlock interest check-in <span>→</span></Link>}{result.paidFeaturesUnlocked ? <Link className="text-button" to={`/compass?result=${encodeURIComponent(result.id)}`}>Explore directions ↗</Link> : <span className="text-button muted">Directions unlock after payment</span>}</div></section><section className="panel timing-panel span-5"><div className="eyebrow">How timing affected this snapshot</div><h2>{result.quality?.status === 'acceptable' ? 'Your session looked steady.' : 'Keep timing in context.'}</h2><p>{result.quality?.warnings?.length ? `The session recorded ${result.quality.warnings.length} quality note${result.quality.warnings.length > 1 ? 's' : ''}. That does not silently change your score, but it is useful context for reading the bars.` : 'No timing warning was recorded. Faster is not automatically better; accuracy and concentration both matter.'}</p><div className="timing-note"><span className="notice-icon">i</span><span>{`Median response times are shown per domain. ${formatTime(metricFor(ranked[0])?.medianResponseTimeMs)}`}</span></div></section></div><AccessibleResultTable result={result} />{result.paidFeaturesUnlocked && <ReportDeliveryCard resultId={result.id} />}
  </div>
}

function AccessibleResultTable({ result }: { result?: AssessmentResult }) {
  if (!result || result.fullAccess === false) return null
  return <section className="panel accessible-data-table" aria-labelledby="accessible-results-title"><details><summary id="accessible-results-title">Read the profile as a table</summary><div className="table-scroll"><table><caption>Provisional IAQ Cognitive Profile strength signals</caption><thead><tr><th scope="col">Rank</th><th scope="col">Thinking area</th><th scope="col">Signal</th><th scope="col">Relative to your average</th></tr></thead><tbody>{[...domains].sort((a, b) => (result.domainScores[b] ?? -1) - (result.domainScores[a] ?? -1)).map((domain, index) => { const signal = result.domainScores[domain]; return <tr key={domain}><td>{signal === null || signal === undefined ? '—' : index + 1}</td><th scope="row">{domain}</th><td>{signal === null || signal === undefined ? 'Not assessed' : `${signal} / 100`}</td><td>{signal === null || signal === undefined ? 'More evidence needed' : signal >= (result.composite ?? 0) ? 'Above your profile average' : 'Below your profile average'}</td></tr> })}</tbody></table></div></details></section>
}
