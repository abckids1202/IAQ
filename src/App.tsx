...
type DomainPreview = { task: string; detail: string; visual: 'abstract' | 'logic' | 'numbers' | 'verbal' | 'spatial' | 'speed' }
...
const domainPreviews: Record<Domain, DomainPreview> = {
  ...
  'Processing speed': { task: 'Compare symbols quickly and choose the exact match.', detail: 'The task rewards accurate comparison while you work under a time limit.', visual: 'speed' },
}
...
function LegacyAssessment({ onComplete }: { onComplete: (scores: Record<Domain, number>) => void }) {
  ...
  const totalQuestions = usingApi ? 40 : questions.length
...
// Legacy memory payloads are rejected by the active six-aspect form. This
// fallback exists only to keep old persisted sessions from crashing the app.
function MemoryTrial({ question, selected, onChoose, disabled }: { question: Question; selected?: string; onChoose: (option: string) => void; disabled: boolean; onBeginRecall?: () => Promise<void>; sessionId?: string | null }) {
  return <div className="option-grid">{question.options.map((option, index) => <button key={option} className={`option ${selected === option ? 'selected' : ''}`} onClick={() => onChoose(option)} disabled={disabled} aria-pressed={selected === option}><span className="option-letter">{String.fromCharCode(65 + index)}</span><span>{option}</span></button>)}</div>
}
...
function Assessment({ onComplete }: { onComplete: (result: AssessmentResult) => void }) {
    ...
    try {
      ...
      if (savedSessionId) {
        try {
          ...
          // create the enforced 40-question form instead of trapping the user.
          window.sessionStorage.removeItem(ASSESSMENT_SESSION_KEY)
  ...
  const beginRecall = async () => undefined
  const next = async () => {
