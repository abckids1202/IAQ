...
function Results({ profile }: { profile: Profile; savedMajors: string[]; toggleMajor: (name: string) => void }) {
  ...
  if (result.fullAccess === false) return <div className="page results-page"><PageIntro eyebrow="Your results / Score ready" title={<>Your score is ready.<br /><em>Unlock the full picture.</em></>} body="You have completed the assessment. The free result shows the experimental IAQ IQ score; the full report adds the seven-domain evidence and practical next steps." /><section className="panel paywall-card"><div className="result-score-head"><div><div className="eyebrow">IAQ IQ score · experimental</div><div className="result-score">{result.iqScore ?? '—'}<span>{result.iqScore === null || result.iqScore === undefined ? 'not enough evidence' : 'reference score'}</span></div></div><span className="confidence-badge">{result.confidence} confidence</span></div><div className="result-disclaimer"><div className="disclaimer-mark">!</div><div><strong>Not an official IQ score</strong><p>{result.disclaimer}</p></div><Link to="/methodology">Read the method ↗</Link></div><h2>{result.paywall?.title || 'Unlock your full report'}</h2><p>{result.paywall?.body || 'Unlock the detailed evaluation, interests, directions, certificate, and private email report.'}</p><div className="secondary-actions"><Link className="button primary" to={`/checkout/${result.paywall?.product_id || 'iaq-complete'}`}>Unlock full report <span>→</span></Link><Link className="text-button" to="/">Return to overview ↗</Link></div></section></div>
