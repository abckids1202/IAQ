...
The cognitive baseline covers abstract reasoning, deductive logic, numerical reasoning, verbal reasoning, visual-spatial reasoning, and working memory. Processing-speed items remain quarantined for research and timing analysis; they are not part of the scored form.

The default complete form selects 8 items from each scored domain (48 questions total) without replacement, avoids duplicate item families where the bank permits it, then shuffles the combined order with a secure random source. The complete session has a 35-minute server-enforced deadline. A new session receives a new form; item IDs, answer keys, and explanations remain server-side.
